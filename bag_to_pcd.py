...
class pcl_sub(Node):
    ...
    def listener_callback(self, msg):
        ...
        xyz = np.array(get_xyz_points(cloud_array))
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(xyz)
        output_destination = os.path.join(self.desination, "pcd_" + str(self.counter) + ".pcd")
        o3d.io.write_point_cloud(output_destination, pcd)
        self.counter += 1
        
        
def main(args=None):
    rclpy.init(args=args)
    pcl_subscriber = pcl_sub('/merged_points', "path_to_destination")
